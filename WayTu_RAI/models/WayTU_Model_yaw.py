...
class GraphPointNet(nn.Module):
    def __init__(self, cfg) :
        ...
        self.conv1 = GraphPointNetLayer(10, 64)
        self.conv2 = GraphPointNetLayer(64, 256)
        self.conv3 = GraphPointNetLayer(256, 512)


        # Currently I am adding a place holder segmentation head
        self.segmentation = Linear(512 + 512, len(self.cfg['label-list-all']))
        # print(len(self.cfg['labels-list']))
        # raise Exception 
    ...
    def forward(self,graph_batch):
        ...
        h = F.relu(h)
        # print("shape3:" , h.shape)      
        global_feat = global_max_pool(h, batch)
        # print(f"global feat: {global_feat.shape}")
        global_feat_expanded = global_feat[batch]
        # print(f"h: {h.shape}")
        # print(f"global feat expended: {global_feat_expanded.shape}")
        combined = torch.cat([h, global_feat_expanded], dim=1)

        # h = global_max_pool(h, graph_batch.batch)
    ...
    def forward(self, graph_batch, objs=None):
        ...
        if objs is not None:
            ...
            for i in range(graph_batch.num_graphs):
                ...
                self.encoder.eval()
                with torch.no_grad():
                    env_feat, yaw = self.encoder(env_pc.unsqueeze(0))
                    env_feat = env_feat.squeeze(0)
                ...
                tool_pc    = gutils.arrange_point_cloud_size(tool_norm,
                                                            self.cfg["feature-size"])
                with torch.no_grad():
                    tool_feat, yaw = self.encoder(tool_pc.unsqueeze(0))
                    tool_feat = tool_feat.squeeze(0)
            ...
            for i in range(B):
                ...
                for t in top3:
                    ...
                    pc_t = gutils.arrange_point_cloud_size(norm_t,
                                                        self.cfg["feature-size"])
                    with torch.no_grad():
                        tool_feat, yaw = self.encoder(pc_t.unsqueeze(0))
                        tool_feat = tool_feat.squeeze(0)
                    ...
                    if best_score is None or score_pred.item() > best_score.item():
                        ...
                        best_yaw = yaw
                        best_rotation = t_params["rotation"]
...
# This model is last proposed model with pretrained segmentation model
# It has one generator had that takes the combination of tool and platforms features 
...
# This is direct replica of previous models waypoint generator head. The layers can be updated.
...
class WayTuModel_old(nn.Module):
    ...
    def forward(self, graph_batch, objs= None):
        ...
        for i in range(graph_batch.num_graphs):
            ...
            # In testing we need to get the environment and all detected tools. 
            else:
